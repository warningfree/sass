  ...
  def visit_extend(node)
    node.resolved_selector.members.each do |seq|
      if seq.members.size > 1
        raise Sass::SyntaxError.new("Can't extend #{seq.inspect}: can't extend nested selectors")
      end
      ...
      if !sseq.is_a?(Sass::Selector::SimpleSequence)
        ...
        raise Sass::SyntaxError.new("Can't extend #{seq.inspect}: can't extend parent selectors")
      end
